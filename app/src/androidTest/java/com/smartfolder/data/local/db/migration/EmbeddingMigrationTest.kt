...
class EmbeddingMigrationTest {
    ...
    @Test
    fun migrate4To5_allowsMultipleModelsPerImage() = runTest {
        createVersion4SchemaWithSingleEmbedding()
        ...
        try {
            db.openHelper.writableDatabase.execSQL(
                """
                INSERT INTO embeddings (imageId, vectorBlob, modelName, createdAt)
                VALUES (10, X'0000803F', 'precise', 2000)
                """.trimIndent()
            )

            val countCursor = db.openHelper.readableDatabase.query(
                "SELECT COUNT(*) FROM embeddings WHERE imageId = 10"
            )
            countCursor.use {
                it.moveToFirst()
            ...
            val fastCursor = db.openHelper.readableDatabase.query(
                "SELECT COUNT(*) FROM embeddings WHERE imageId = 10 AND modelName = 'fast'"
            )
            fastCursor.use {
                it.moveToFirst()
            ...
            val preciseCursor = db.openHelper.readableDatabase.query(
                "SELECT COUNT(*) FROM embeddings WHERE imageId = 10 AND modelName = 'precise'"
            )
            preciseCursor.use {
                it.moveToFirst()
                assertEquals(1, it.getInt(0))
    ...
    private fun createVersion4SchemaWithSingleEmbedding() {
        ...
        try {
            ...
            sqlite.execSQL(
                "INSERT INTO embeddings (id, imageId, vectorBlob, modelName, createdAt) " +
                    "VALUES (100, 10, X'0000803F', 'fast', 1000)"
            )
    ...
    companion object {
        private const val TEST_DB_NAME = "migration-embeddings-test.db"
    }
}
